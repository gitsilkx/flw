/*
* NEW BrowserCompAPI version 1.0.0.19
...
if (/___requireStorage/.test(document.location.href) || myconduitAppData || typeof (abstractionlayer) !== "undefined") {
...
var CUSTOM_DIV_ID = "__conduitCustomDiv";
...
// Creating custom div for chrome comm.
...
var resultData = result && result.data ? JSON.parse(result.data) : "{}";
if (callback && typeof callback === 'function') {
console.info("ResultData: ", resultData, result, listenerSender, callback);
...
try {
if (typeof (EBKeyChanged) !== "undefined") {
...
try {
if (typeof (EBGlobalKeyChanged) !== "undefined") {
EBGlobalKeyChanged(key, value);
}
} catch (generalException) {
}
...
main: function (href) {
conduit.tabs.update(data.currentTab.tabId, { url: href });
},
top: function (href) {
...
currentTab: conduit.currentApp.tabInfo
};
setTimeout(function () {
...
conduit.app.embedded.setEmbedded({ width: parseInt(newWidth, 10) });
...
this.getInfo = function () { return JSON.stringify(data.info); };
this.getMainFrameTitle = function () {
...
"alert('TPI: ' + typeof(TPI)); ",
" })();",
" }"
].join("");
conduit.tabs.executeScript(tabId || null, { code: appendToolbarApi });
conduit.tabs.executeScript(tabId || null, { code: "function EBCallBackMessageReceived(data){ alert('EBCallbackMessageReceived is not implemented yet!'); }" });
}
...
this.navigateInMainFrame = function (url) {
conduit.tabs.update(data.currentTab.tabId, { url: url });
};
this.navigateInNewTab = function (url) {
