/*
 * @name Odesk Beauty Tips
 * @modified version of jquery.bt plugin 
...
// oDesk fixed BUG 21047, BUG 22146

//20100614  - updated to prevent hiding tip when it is hovered over 
 
jQuery.obt = {version: '0.9.5-rc1'};
...
(function($) { 
   ...
   * $(selector).obt();
   * $(selector).obt('Content text');
   * $(selector).obt('Content text', {option1: value, option2: value});
   * $(selector).obt({option1: value, option2: value});
   *
   * For more/better documentation and lots of examples, visit the demo page included with the distribution
   *
   */ 
  
  jQuery.fn.obt = function(content, options) {
    ...
    if (jQuery.fn.hoverIntent && jQuery.obt.defaults.trigger == 'hover') {
      jQuery.obt.defaults.trigger = 'hoverIntent';
    }
    $(this).data('obt', true);
    return this.each(function(index) {
  
      var opts = jQuery.extend(false, jQuery.obt.defaults, jQuery.obt.options, options);

      // clean up the options
      opts.spikeLength = numb(opts.spikeLength);
      opts.spikeGirth = numb(opts.spikeGirth);
      opts.overlap = numb(opts.overlap);
      opts.top_indent = numb(opts.top_indent);
      
      var ajaxTimeout = false;
      
      /**
       * This is sort of the "starting spot" for the this.each()
       * These are the init functions to handle the .obt() call
       */
      ...
      $(window).bind('resize', function() {
          if ($(self).hasClass('bt-active')) {
              self.obtOn(null, true);
          }
      });
      
      var timer, showBt;
      var checkTimer, $box, $text;
      var self = this;
      /**
       *  The BIG TURN ON
       *  Any element that has been initiated
       */
      this.obtOn = function (coords, reposition) {
        if (!reposition) {
            if(opts.ajaxPrefetchOffset != null && !timer && !showBt) timer = setTimeout(function() {
                showBt = true;
                timer = false;
                self.obtOn(coords);
            }, opts.ajaxPrefetchOffset);
            if(opts.ajaxPrefetchOffset != null && timer && !showBt) return;
            ...
            // turn off other tips
            $(jQuery.obt.vars.closeWhenOpenStack).obtOff();
            ...
            if (contentSelect && opts.ajaxPath == null) {
              ...
              if(opts.contentSelector == "$(this).attr('title')") {
                content = $(this).attr('bt-xTitle');
              } else {
                content = $.isFunction(opts.contentSelector) ? opts.contentSelector.apply(this) : eval(opts.contentSelector);
              }
            }
        ...
        if (!reposition) checkTimer = setInterval(function(){
          if(!self.offsetWidth) $(self).obtOff();
        }, 100);
        ...
        var fixedOffset = {top: 0, left: 0};
        $(this).parents().each(function() {
            if($(this).css('position') == 'fixed') {
                fixedOffset = {top: $(document).scrollTop(), left: $(document).scrollLeft()};
                return false;
            }
        });
        top += fixedOffset.top;
        ...
        if (!reposition) {
            ...
            $text = $('<div class="bt-content"></div>').append(content).css({padding: opts.padding, position: 'absolute', width: (opts.shrinkToFit ? 'auto' : opts.width), zIndex: opts.textzIndex, left: shadowShiftX, top: shadowShiftY}).css(opts.cssStyles);
            // create the wrapping box which contains text and canvas
            // put the content in it, style it, and append it to the same offset parent as the target
            $box = $('<div class="bt-wrapper"></div>').append($text).addClass(opts.cssClass).css({position: 'absolute', width: opts.width, zIndex: opts.wrapperzIndex, visibility:'hidden'}).appendTo(document.body).data('element', $(this));
        ...
        if(coords) {
            space.top = coords.y - scrollTop;
            space.bottom = docHeight  + scrollTop - coords.y;
            space.left = coords.x - scrollLeft;
            space.right = docWidth + scrollLeft - coords.x;
        } else {
        ...
        var textOutHeight = numb($text.btOuterHeight());
        ...
        if(opts.vertical != 'auto') {
            vert = top + opts.vertical;
        }
        var points = [];
        ...
        switch(position) {
          ...
          case 'top':
            ...
            $box.css({top: (top - $text.btOuterHeight(true)) + opts.overlap, left: horiz});
            if(coords) {
                if(coords.x) $box.css('left', coords.x - textOutWidth/2);
                if(coords.y) $box.css('top', coords.y - textOutHeight + opts.overlap);
            }
            ...
            textBottom = textTop + $text.btOuterHeight();
            textCenter = {x: textLeft + ($text.btOuterWidth()*opts.centerPointX), y: textTop + ($text.btOuterHeight()*opts.centerPointY)};
            // points[points.length] = {x: x, y: y};
            points[points.length] = spikePoint = {y: textBottom + opts.spikeLength, x: ((textRight-textLeft) * .5) + xShift, type: 'spike'};
            if(opts.vertical != 'auto' || coords) {
                textCenter.x = spikePoint.x;
            }
          ...
          case 'left':
            ...
            $box.css({top: vert + opts.top_indent + 'px', left: ((left - $text.btOuterWidth(true)) + opts.overlap) + 'px'});
            if(opts.atMouse) {
                $box.css({
                    left: $.mousePosition.x - numb($text.btOuterWidth(true)) - 5,
                    top: $.mousePosition.y - opts.vertical
                });
            }
            if(coords) {
                if(coords.x) $box.css('left', coords.x - textOutWidth + opts.overlap);
                if(coords.y) $box.css('top', coords.y - textOutHeight/2);
            } 
            if(opts.vertical != 'auto') {
                $box.css('top', top + this.offsetHeight/2  - opts.vertical);
            }
            // move text up/down if extends out of window
            textBottomSpace = (winBottom - opts.windowMargin) - ($text.offset().top + $text.btOuterHeight(true));
            var yShift = shadowShiftY - opts.top_indent;
            if(opts.vertical != 'auto') {
                yShift = -$text.btOuterHeight()/2 + opts.vertical;
            }
            if(coords) yShift = 0;
          ...
          case 'bottom':
            ...
            if(coords) {
                if(coords.x) $box.css('left', coords.x - textOutWidth/2);
                if(coords.y) $box.css('top', coords.y - opts.overlap);
            }
            textRightSpace = (winRight - opts.windowMargin) - ($text.offset().left + $text.btOuterWidth(true));
            var xShift = shadowShiftX;
            if (textRightSpace < 0) {
          ...
          case 'right':
            ...
            $box.css({top: vert + opts.top_indent + 'px', left: ((left + width) - opts.overlap) + 'px'});
            if(opts.atMouse) {
                $box.css({
            ...
            if(coords) {
                if(coords.x) $box.css('left', coords.x - opts.overlap);
                if(coords.y) $box.css('top', coords.y - textOutHeight/2);
            }
            if(opts.vertical != 'auto') {
                $box.css('top', top + this.offsetHeight/2  - opts.vertical);
        ...
        $(canvas).attr('width', (numb($text.btOuterWidth(true)) + opts.strokeWidth*2 + shadowMarginX)).attr('height', (numb($text.btOuterHeight(true)) + opts.strokeWidth*2 + shadowMarginY)).appendTo($box).css({position: 'absolute', zIndex: opts.boxzIndex});
        ...
        //oDesk bug 37464 - if preShow return false - do not show toolTip - to provide conditional appearance of tip.
        var res = opts.preShow.apply(this, [$box[0]]);
        if (false === res) return;
        //20100614 - added ability to detect if mouse is over box 
        if (opts.keepAlive){
            $box.hover(
        ...
        if (opts.clickAnywhereToClose) {
          jQuery.obt.vars.clickAnywhereStack.push(this);
          $(document).click(jQuery.obt.docClick);
        }
        
        // stick this element into the closeWhenOthersOpen stack
        if (opts.closeWhenOthersOpen) {
          jQuery.obt.vars.closeWhenOpenStack.push(this);
        }
        
        //20100614 - added functionality to bind tip content to mouseleave event
        if(opts.keepAlive){
            $('.bt-content').bind('mouseleave',function() {
                $box.removeData('hover');
                self.obtOff();
            });
        }
      ...
      this.obtOff = function() {
        clearInterval(checkTimer);
        var box = $(this).data('bt-box') || ($box ? $box[0] : null);
        //20100614 added check for .data('hover') to return if box is being hovered over 
        if ($(box).data('hover')) {
          return;
        }
        clearTimeout(timer);
        ...
        i.btCleanup = function(){
          ...
          jQuery.obt.vars.clickAnywhereStack = arrayRemove(jQuery.obt.vars.clickAnywhereStack, i);
          jQuery.obt.vars.closeWhenOpenStack = arrayRemove(jQuery.obt.vars.closeWhenOpenStack, i);
  ...
  */
  ...
  jQuery.fn.btOuterHeight = function(margin) {
      ...
      + (margin ? num(this, "marginTop")
      + num(this, "marginBottom") : 0);
  
  }; // </ jQuery.fn.btOuterHeight() >
  
  /**
   * A convenience function to run obtOn() (if available)
   * for each selected item
   */
  jQuery.fn.obtOn = function(coords) {
    return this.each(function(index){
      if (jQuery.isFunction(this.obtOn)) {
        this.obtOn(coords);
      }
    });
  }; // </ $().obtOn() >
   ...
   * for each selected item
   */
  jQuery.fn.obtOff = function() {
    return this.each(function(index){
      if (jQuery.isFunction(this.obtOff)) {
        this.obtOff();
      }
    });
  }; // </ $().obtOff() >
  
  jQuery.obt.vars = {clickAnywhereStack: [], closeWhenOpenStack: []};
  
  /**
   * This function gets bound to the document's click event
   * It turns off all of the tips in the click-anywhere-to-close stack
   */
  jQuery.obt.docClick = function(e) {
    if (!e) {
      var e = window.event;
    };
    // if clicked element is a child of neither a tip NOR a target
    // and there are tips in the stack
    if (!$(e.target).parents().andSelf().filter('.bt-wrapper, .bt-active').length && jQuery.obt.vars.clickAnywhereStack.length) {
      // if clicked element isn't inside tip, close tips in stack
      $(jQuery.obt.vars.clickAnywhereStack).obtOff();
      $(document).unbind('click', jQuery.obt.docClick);
    }
  }; // </ docClick() >
   ...
   *
   *   jQuery.obt.options.width = 400;
   *
   * Be sure to use *jQuery.obt.options* and not jQuery.obt.defaults when overriding
   *
  ...
  jQuery.obt.defaults = {
    trigger:         'hover',                // trigger to show/hide tip
                                             ...
                                             //   'none'            // use $('#selector').obtOn(); and ...obtOff();
    ...
    width:            '300px',               // width of tooltip box
    
    padding:          '16px',                // padding for content (get more fine grained with cssStyles)
    ...
    textzIndex:       5000002,                  // z-index for the text
    boxzIndex:        5000001,                  // z-index for the "talk" box (should always be less than textzIndex)
    wrapperzIndex:    5000000,
                                             // Must be positioned relative or absolute. Can be selector or object
    positions:        ['right','left'],              // preference of positions for tip (will use first with available space)
                                             // possible values 'top', 'bottom', 'left', 'right' as an array in order of
                                             // preference. Last value will be used if others don't have enough space.
                                             // or use 'most' to use the area with the most space
    fill:             "#E5F6FD",  // fill color for the tooltip box, you can use any CSS-style color definition method
                                             // http://www.w3.org/TR/css3-color/#numerical - not all methods have been tested
    
    windowMargin:     10,                    // space (px) to leave between text box and browser edge
  
    strokeWidth:      1,                     // width of stroke around box, **set to 0 for no stroke**
    strokeStyle:      "#7FD7F7",                // color/alpha of stroke
  
    cornerRadius:     6,                     // radius of corners (px), set to 0 for square corners
    
                      // following values are on a scale of 0 to 1 with .5 being centered
    
    centerPointX:     .5,                    // the spike extends from center of the target edge to this point
    centerPointY:     .5,                    // defined by percentage horizontal (x) and vertical (y)
    top_indent:       0,                     // indent from the top (available only for left/right tip positions)
    ...
    hoverIntentOpts:  {                          // options for hoverIntent (if installed)
                        interval: 400,           // http://cherne.net/brian/resources/jquery.hoverIntent.html
                        timeout: 400
                      },
  ...
  $(document).ready(function() {
      $(document).bind('mousemove', function(event) {
          ...
          $.mousePosition.x += $(document).scrollLeft() - scrollLeft;
          $.mousePosition.y += $(document).scrollTop() - scrollTop;
          scrollLeft = $(document).scrollLeft();
          scrollTop = $(document).scrollTop();
      });
  });
  
  // Bug 31563: IE VML events fix
  if (jQuery.browser.msie && !jQuery.support.changeBubbles) {
    var originFunc = jQuery.event.special.change.filters.click;
    jQuery.event.special.change.filters.click = function(e){
        var elem = e.target,
            nodeName = elem.nodeName.toLowerCase();
        if (nodeName === "input" || nodeName === "select") {
            originFunc.apply(this, [e]);
        }
    };
  }
