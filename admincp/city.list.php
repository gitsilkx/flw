...
include("includes/header.php");
 ...
 /************************Delete***************************************************/	
if($_GET['del']):
//echo "delete from " .$prev."city where city_id=" . $_GET['city_id'];
//die();
  mysql_query("DELETE FROM " .$prev."city WHERE city_id=" . $_GET['city_id']);
  $_SESSION['succ_message']="City Deleted Successfully !!";
...
if(isset($_REQUEST['status'])){
	   $update=mysql_query("UPDATE ".$prev."city SET status='".$_REQUEST['status']."' WHERE city_id=".$_REQUEST['city_id']);	
	}
 ...
 /************************Search***************************************************/	
  $sortby='';$cond=array();$parama="";
	...
	if($_POST[sortby])
	{
	   ...
	   if($_POST[sortby]=="rasc"){$sortby=" ORDER BY city_name ASC";}
	   if($_POST[sortby]=="rdesc"){$sortby=" ORDER BY city_name DESC";}
...
/***********************End Search**************************/
  ...
  $sql="SELECT * FROM ".$prev."city WHERe 1 ".$conds." ".$sortby;
...
function del()
{	
	if(confirm('Are you sure you want to delete all Cities?')==true)
	{
		document.formListContainer.method="post";
		document.formListContainer.action="delete.php?city_id=city_id";
...
<div class="adminRight">
	<div id="loadingrefresh">
		<div class="rightContHeading Heading">Manage City</div>
		<div class="rightContBody">
			<div class="riteContWrap1">	
							<!-- Sort By -->
                <div class="manageButtonLeft marginBot">
			<form name="restaurantmanage" method="post" action="city.list.php?menuid=115&menupid=113" />
			...
			<select class="restManageNameDrop" name="sortby" id="sortby" size="1" onchange="document.restaurantmanage.submit();">
				...
				<optgroup label="Others">
                    <option value="rasc" <? if($_POST[sortby]=="rasc"){echo "selected";}?>>&nbsp;&nbsp;City Name A to Z</option>
			        <option value="rdesc" <? if($_POST[sortby]=="rdesc"){echo "selected";}?>>&nbsp;&nbsp;City Name Z to A</option>
				...
				<div class="manageButtonLastCont">
					...
					<a class="manageButton_addnw" href="city.entryform.php?menuid=116&menupid=113">Add New</a>
				...
				<tr class="listHeader">
					...
					<td width="7%" align="center" class="listHeaderCont"><a onclick="sortByAscDesc('zdesc','','');" href="javascript:void(0);">S.No</a></td>
					<td width="10%" align="left" class="listHeaderCont">
						<a onclick="sortByAscDesc('zdesc','','');" href="javascript:void(0);">City Name						</a>
				  </td>
                                 <td width="20%" align="left" class="listHeaderCont"><a onclick="sortByAscDesc('zdesc','','');" href="javascript:void(0);">City ID</a></td>
                                  <td width="5%" align="center" class="listHeaderCont">State Name</td>
                                  <td width="5%" align="center" class="listHeaderCont">Url</td>
                ...
                <?php if($totalrow){
					...
					<td align="center" class="listCont"><span class="listContResta">
					  <input type='checkbox' name="chk[]" id="checkbox_id_<?=$row->city_id ?>" class="checkbox_id"  value="<?php echo $row->city_id?>"/>
					</span></td>
					<td align="center" class="listCont"><?=$i?></td>
					
                    <td align="left" class="listCont"><?=$row->city_name?></td>
					<td align="left" class="listCont"><?=$row->city_id?></td>					
					<td align="left" class="listCont"><?=$row->state?></td>
                                        <td align="left" class="listCont"><?=$row->url?></td>
					<td align="center" class="listCont" id="chgstatus1987">
					<?php if($row->status=='Y'){?>
                     <a href="city.list.php?menuid=115&menupid=113&city_id=<?=$row->city_id?>&status=N">
                        <img src='images/icon_active.png' alt='Active' title='Active' style="cursor:pointer;"  />
                    </a>
                    <?php }else{?>
                    <a href="city.list.php?menuid=115&menupid=113&city_id=<?=$row->city_id?>&status=Y">
                   <img src='images/delete.png' alt='Inactive' title='Inactive' style="cursor:pointer;"  />
					...
					<td align="center" class="listCont">
						<span class="EditDeleteButton">
							<a href="city.entryform.php?menuid=115&menupid=113&city_id=<?php echo $row->city_id; ?>">
								<img width="16" height="16" title="Edit" alt="Edit" src="images/icon_edit.png">
							</a>
						</span>
						<span class="EditDeleteButton">
                            <a href="<?=$_SERVER['PHP_SELF']?>?city_id=<?=$row->city_id?>&del=1" class='lnk'  onclick="return confirm('Are you sure you want to delete this record?')">
				...
				<!--Button List start-->
				<div class="manageButtonLastCont">
					<a class="manageButton_addnw thickbox" href="city.entryform.php?menuid=116&menupid=113">Add New</a>
					<input type="button"  class="manageButton but_activate" value="Activate" onclick="adminActivateDeactivate('1','status','maincateid','rt_category_main','Main Category');" style="display:none;"/>
					<input type="button" class="manageButton but_deactivate" value="Deactivate" style="display:none;" onclick="adminActivateDeactivate('0','status','maincateid','rt_category_main','Main Category');" />
