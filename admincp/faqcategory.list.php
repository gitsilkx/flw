...
//	echo "select * from " . $prev . "faq where id=" . $_GET[id];
   $r=mysql_query("select * from " . $prev . "faq where faq_cat=" . $_GET[id]);
   if(@mysql_num_rows($r)):
   //echo "aa";
...
<p class="lnk" align="right">&nbsp;
	...
	&nbsp;|&nbsp;
	<u><img src='images/icon_add_sub_page.gif' border='0'></u>&nbsp;= Add Sub Page</p>
...
while($d=@mysql_fetch_array($r)):
    ...
    if(in_array($d[id],$tids)):
		...
		while($dd=mysql_fetch_array($rr)):
			...
			<td align=center> <a class=lnk href='faqcategory.entryform.php?parent_id=" .$d[id]. "&tid=".$_GET[tid]."&id=" . $dd[id] . "'><u>".$main_icon_edit_img."</u></a> | <a class=lnk href='faq.list.php?faq_cat=" . $dd[id] . "'><u>View FAQ</u></a> | <a class=lnk href='faq.entryform.php?id=" . $dd[id] . "'><u>Add FAQ</u></a> | <a class=lnk href=\"javascript:if(confirm('DO you want to delete?')){window.location='" . $_SERVER['PHP_SELF'] . "?tid=" . $_GET[tid] . "&id=" . $dd[id] . "&del=1';}\"><u>Delete</u></a></td></tr>";
        endwhile;
	endif;	
	$j++;
endwhile;   
