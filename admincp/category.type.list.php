...
/************************Delete***************************************************/	
if($_REQUEST['del']):
  mysql_query("delete from " .$prev."category where cat_id=" . $_GET['cat_id']);
...
if(isset($_REQUEST['status'])){
	   $update=mysql_query("UPDATE ".$prev."category SET status='".$_REQUEST['status']."' WHERE cat_id=".$_REQUEST['cat_id']);	
	}
 ...
 /************************Search***************************************************/	
  $sortby='';$cond=array();$parama="";
	...
	if($_REQUEST[sortby])
	{
	   ...
	   if($_REQUEST[sortby]=="rasc"){$sortby=" order by cat_name asc";}
	   if($_REQUEST[sortby]=="rdesc"){$sortby=" order by cat_name desc";}
	    if($_REQUEST[sortby]=="1"){$sortby=" order by cat_option asc";}
	   if($_REQUEST[sortby]=="2"){$sortby=" order by cat_option desc";}
...
/*************************End pagination*******************************/	
  ...
  $sql="SELECT * FROM ".$prev."category where 1 ".$conds." ".$sortby;
...
{	
	if(confirm('Are you sure you want to delete Category?')==true)
	{
		...
		document.formListContainer.action="delete.php?cat_id=cat_id";
...
<div class="adminRight">
	<div id="loadingrefresh">
		...
		<div class="rightContBody">
			...
			<form name="offermanage" method="post" action="category.type.list.php?menuid=122&menupid=120" />
			...
			<select class="restManageNameDrop" name="sortby" id="sortby" size="1" onchange="document.offermanage.submit();">
				...
				<div class="manageButtonLastCont">
					...
					<a class="manageButton_addnw" href="rest_category.php?menuid=121&menupid=120&rest_id=<?=$_REQUEST['rest_id']?>">Add New</a>
			...
			<table id="tableListContent" width="100%" class="sort-table" border="0" align="center" cellspacing="1" cellpadding="4"  style="border:solid 1px <?=$dark?>">
					...
					<td width="10%" align="center" class="listHeaderCont">S.No</td>
					<td width="15%" align="left" class="listHeaderCont">
						<a href="javascript:void(0);" onclick="sortByAscDesc('perdesc','','');">Category ID						</a>
					</td>
					<td width="20%" align="left" class="listHeaderCont">
						<a href="javascript:void(0);" onclick="sortByAscDesc('pricedesc','','');">Category Name </a>
					</td>
                    <td width="20%" align="left" class="listHeaderCont">
						<a href="javascript:void(0);" onclick="sortByAscDesc('pricedesc','','');">Category Option </a>
					</td>
					<td width="20%" align="left" class="listHeaderCont"><a href="javascript:void(0);" onclick="sortByAscDesc('rdesc','','');">Restaurant Name	</a>
  </td>
										<td width="15%" align="center" class="listHeaderCont"><a href="javascript:void(0);" onclick="sortByAscDesc('perdesc','','');">Added Date</a></td>
					<td width="5%" align="center" class="listHeaderCont"><a href="javascript:void(0);" onclick="sortByAscDesc('rdesc','','');">Status</a></td>
					...
					<td align="center" class="listCont"><span class="listContResta">
					  <input type='checkbox' name="chk[]" id="checkbox_id_<?=$row->cat_id ?>" class="checkbox_id"  value="<?php echo $row->cat_id?>"/>
					</span></td>
					<td align="center" class="listCont"><?=$i?></td>
					<td align="left" class="listCont"><?php echo $row->cat_id; ?></td>
					<td align="left" class="listCont"><?php echo $row->cat_name; ?></td>
                    <td align="left" class="listCont"><?php echo $row->cat_option; ?></td>
					...
					<?php if($row->status=='Y'){?>
                     <a href="category.type.list.php?menuid=122&menupid=120&cat_id=<?=$row->cat_id?>&status=N">
                        <img src='images/icon_active.png' alt='Active' title='Active' style="cursor:pointer;"  />
                    </a>
                    <?php }else{?>
                    <a href="category.type.list.php?menuid=122&menupid=120&cat_id=<?=$row->cat_id?>&status=Y">
                   <img src='images/delete.png' alt='Inactive' title='Inactive' style="cursor:pointer;"  />
					...
					<!--<td align="center" class="listCont" id="chgstatus5">
                    =($row->status=='Y')?'<img src="images/icon_active.png" alt="Active" title="Active"  border="0" style="cursor:pointer;" />':'<img src="images/icon_inactive.gif" alt="Active" title="Active" border="0" style="cursor:pointer;" />'?>
												
						  </td>-->
					<td align="center" class="listCont">
						<span class="EditDeleteButton">
							<a href="rest_category.php?menuid=121&menupid=120&cat_id=<?=$row->cat_id?>">
								<img src="images/icon_edit.png" width="16" height="16" border="0" alt="Edit" title="Edit" />
							</a>
						</span>
						<span class="EditDeleteButton">
                    <a href="<?=$_SERVER['PHP_SELF']?>?cat_id=<?=$row->cat_id?>&del=1" class='lnk'  onclick="return confirm('Are you sure you want to delete?')">
				...
				<!--Button List start-->
				<div class="manageButtonLastCont">
					<a class="manageButton_addnw thickbox" href="rest_category.php?menuid=121&menupid=120">Add New</a>
					<input type="button"  class="manageButton but_activate" value="Activate" onclick="adminActivateDeactivate('1','status','maincateid','rt_category_main','Main Category');" style="display:none;"/>
					<input type="button" class="manageButton but_deactivate" value="Deactivate" style="display:none;" onclick="adminActivateDeactivate('0','status','maincateid','rt_category_main','Main Category');" />
