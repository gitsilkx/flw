...
/************************Delete***************************************************/	
if($_REQUEST['del']):
  mysql_query("delete from " .$prev."addons where addons_id=" . $_GET['addons_id']);
  mysql_query("delete from " .$prev."sub_addons where addons_id=" . $_GET['addons_id']);
  $_SESSION['succ_message'] = "Addons Deleted Successfully !!";
...
if(isset($_REQUEST['status'])){
	   $update=mysql_query("UPDATE ".$prev."addons SET status='".$_REQUEST['status']."' WHERE addons_id=".$_REQUEST['addons_id']);	
	}

 /************************Search***************************************************/	
  $sortby='';$cond=array();$parama="";

	if($_REQUEST['searchofferid']){

		$cond[]="rest_id='".$_REQUEST['searchofferid']."'";
		//$parama.="&product_brand=".$_REQUEST[brand_id];
	}
	else if($_REQUEST['rest_id'])
	{
		$cond[]="restaurent_id='".$_REQUEST['rest_id']."'";
	}
	...
	if($_REQUEST[sortby])
	{
	   ...
	   if($_REQUEST[sortby]=="rasc"){$sortby=" order by addons_name asc";}
	   if($_REQUEST[sortby]=="rdesc"){$sortby=" order by addons_name desc";}
...
/*************************End pagination*******************************/	
  ...
  $restau = "SELECT rest_id FROM".$prev."restaurant";
  $res_restau = mysql_query($restau);
  $sql="SELECT * FROM ".$prev."addons where 1 ".$conds." ".$sortby;
...
function del()
{	
	if(confirm('Are you sure you want to delete Addons and Subaddons?')==true)
	{
		document.formListContainer.method="post";
		document.formListContainer.action="delete.php?addons_id=addons_id";
...
<div class="adminRight">
	<div id="loadingrefresh">
		<div class="rightContHeading Heading">Manage Addons</div>
		<div class="rightContBody">
			<div class="riteContWrap1">	
							<!-- Sort By -->
                <div class="manageButtonLeft marginBot">
			<form name="offermanage" method="post" action="addons.list.php?menuid=127&menupid=125" />
			<!--<select class="restManageNameDrop" name="searchofferid" id="searchoffer" onchange="document.offermanage.submit();">
				<option value="">Select offer Name</option>
				<?php 
				//$result1=mysql_query("SELECT `rest_id`,`rest_name` FROM ".$prev."addons");
				//$totno=mysql_num_rows($result1);
				//if($totno){
				//while($row1=mysql_fetch_object($result1)){
                ?>
                <option value="<?//= $row1->rest_id?>" <?php //if($_REQUEST['searchofferid']==$row1->rest_id)//{?> selected="selected"<?php //}?> ><?//= $row1->rest_name?></option>
			...
			<select class="restManageNameDrop" name="sortby" id="sortby" size="1" onchange="document.offermanage.submit();">
				...
				<div class="manageButtonLastCont">
					...
					<a class="manageButton_addnw" href="add.new.addons.php?menuid=127&menupid=125&rest_id=<?=$_REQUEST['rest_id'];?>">Add New</a>
			...
			<table id="tableListContent" width="100%" class="sort-table" border="0" align="center" cellspacing="1" cellpadding="4"  style="border:solid 1px <?=$dark?>">
					...
					<td width="15%" align="left" class="listHeaderCont">
						<a href="javascript:void(0);" onclick="sortByAscDesc('perdesc','','');">Addons ID						</a>
					</td>
					<td width="10%" align="left" class="listHeaderCont">
						<a href="javascript:void(0);" onclick="sortByAscDesc('pricedesc','','');">Addons Name						</a>
					</td>
					<td width="15%" align="left" class="listHeaderCont"><a href="javascript:void(0);" onclick="sortByAscDesc('rdesc','','');">Category Name						</a>
  </td>
										<td width="15%" align="center" class="listHeaderCont"><a href="javascript:void(0);" onclick="sortByAscDesc('perdesc','','');">Restaurant Name</a></td>
                                <td width="5%" align="center" class="listHeaderCont">Added Date</td>
					...
					<td align="center" class="listCont"><span class="listContResta">
					  <input type='checkbox' name="chk[]" id="checkbox_id_<?=$row->addons_id ?>" class="checkbox_id"  value="<?php echo $row->addons_id?>"/>
					</span></td>
					<td align="center" class="listCont"><?=$i?></td>
					<td align="left" class="listCont"><?php echo $row->addons_id; ?></td>
					<td align="left" class="listCont"><?php echo $row->addons_name; ?></td>
					<td align="left" class="listCont"><?php echo getCategoryId($row->cat_id); ?></td>
				    <td align="center" class="listCont"><?php echo getRestuarantnameById($row->restaurent_id); ?></td>
                    <td align="center" class="listCont"><?php echo $row->cur_date; ?></td>
					<td align="center" class="listCont" id="chgstatus1987">
					<?php if($row->status=='Y'){?>
                     <a href="addons.list.php?menuid=127&menupid=125&addons_id=<?=$row->addons_id?>&status=N">
                        <img src='images/icon_active.png' alt='Active' title='Active' style="cursor:pointer;"  />
                    </a>
                    <?php }else{?>
                    <a href="addons.list.php?menuid=127&menupid=125&addons_id=<?=$row->addons_id?>&status=Y">
                   <img src='images/delete.png' alt='Inactive' title='Inactive' style="cursor:pointer;"  />
					...
					<td align="center" class="listCont">
						<span class="EditDeleteButton">
							<a href="add.new.addons.php?menuid=126&menupid=125&addons_id=<?=$row->addons_id?>">
								<img src="images/icon_edit.png" width="16" height="16" border="0" alt="Edit" title="Edit" />
							</a>
						</span>
						<span class="EditDeleteButton">
                    <a href="<?=$_SERVER['PHP_SELF']?>?addons_id=<?=$row->addons_id?>&del=1" class='lnk'  onclick="return confirm('Are you sure you want to delete?')">
				...
				<!--Button List start-->
				<div class="manageButtonLastCont">
					<a class="manageButton_addnw thickbox" href="add.new.addons.php?menuid=127&menupid=125">Add New</a>
					<input type="button"  class="manageButton but_activate" value="Activate" onclick="adminActivateDeactivate('1','status','maincateid','rt_category_main','Main Category');" style="display:none;"/>
					<input type="button" class="manageButton but_deactivate" value="Deactivate" style="display:none;" onclick="adminActivateDeactivate('0','status','maincateid','rt_category_main','Main Category');" />
