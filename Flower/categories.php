...
$parama = "";
$order_by = 'A.name asc';
//header("Location: https://www.flowerwyz.com/categories.php?category=apop&pg_id=7");
//header('Location: http://localhost/flowerwyz/flower-shops-online-flower-stores.htm', true, 301);
...
if ($_GET['category'] && $_GET['category'] != '' && $_GET['pg_id'] != '') {
    ...
    elseif ($strCategory == 'apop' && $page_id == '4') //Today
        $field_name = 'today';
    elseif ($strCategory == 'apop' && $page_id == '5') //Next Day
        $field_name = 'tomorrow';
    elseif ($strCategory == 'apa' && $page_id == '6') //Deal
        $field_name = 'deals';
    ...
    elseif ($strCategory == 'o' && $page_id == '40') //Arrangements
...
if (!empty($b)) {
    ...
    if ($c[0] == 'limit') {
        ...
        $array = explode('_', $c[1]);
        $filter = $c[1];
        $order_by = "A." . $array[0] . " " . $array[1];
        $parama = "&amp;category=" . $_GET['category'] . "&amp;pg_id=" . $_GET['pg_id'] . "&amp;filter=" . $c[1];
    } elseif ($c[0] == 'range') {
        if ($c[1] == '40-60')
        ...
        $parama = "&amp;category=" . $_GET['category'] . "&amp;pg_id=" . $_GET['pg_id'] . "&amp;range=" . $c[1];
    }
    if ($c1[0] == 'filter') {

        $array = explode('_', $c1[1]);
        $filter = $c1[1];
        $order_by = "A." . $array[0] . " " . $array[1];
        $parama = "&amp;category=" . $_GET['category'] . "&amp;pg_id=" . $_GET['pg_id'] . "&amp;filter=" . $c1[1];
    }
    if ($c2[0] == 'range') {
        if ($c2[1] == '40-60')
            $where_clause .= " AND A.price>=40 AND A.price<=60";
    ...
    if ($c3[0] == 'category') {
       ...
       // $order_by = "B." . $field_name;
        ...
        $parama = "&amp;category=" . $c3[1] . "&amp;pg_id=" . $c4[1];
    }
}
...
if ($_GET['pg_id'] && $_GET['pg_id'] != '') {
    $page_id = $_GET['pg_id'];
    if ($page_id == '1') {
        $keywords = array('Wedding Anniversary Gifts', 'Anniversary Gift Ideas', 'Anniversary Flowers', 'Flowers For Anniversary', 'Wedding Anniversary Flowers',
        ...
        $keywords = array('Flowerwyz Flower Shop', 'Flower Arrangements', 'Best Flower Shops', 'Cool Flower Stores', 'Flower Arrangement',
            'Flowers Arrangements', 'Flowers Shop Flowerwyz', 'The Flower Shop', 'Smooth Flowershop Online', 'Flower Arrangements Ideas', 'Flowers Shops',
            'Cheap Flower Arrangements', 'Modern Floral Arrangements', 'Flowerwyz Flower Stores', 'Flowershops', 'Flower Shops That Deliver', 'Online Flower Shop', 'Flower Shop Delivery',
            'Flower Shops Online', 'Flower Arrangements Delivery');
    } elseif ($page_id == '9') {
        ...
        $keywords = array('Best Floral Arrangements', 'Best Florists & Designers', 'Best Floral Shops', 'Fastest Florist Delivery', 'Best Online Florists',
            'Unique Floral Expressions', 'Coveted Florist Shops', "Online Florists' Bouquets", 'Online Florist Designs', 'Best Florist Supplies',
            'Best Floral Delivery', 'Floral Arrangement Ideas', 'Spring Floral Arrangements', 'Florists Online', 'Cheap Floral Arrangements',
            'Unique Floral Arrangements', 'Large Floral Arrangements', 'Dried Floral Arrangements', 'Tropical Floral Arrangements', 'Cheap Florist Flowers');
...
<?php if (CheckCron() == 0) { ?>
                <div class="filterPan">
                    <ul class="listing">
                        ...
                        <li class="bt40-60"><a href="<?= $page; ?>?range=40-60">UNDER $60</a></li>
                        <li class="bt60-80"><a href="<?= $page; ?>?range=60-80">$60 - $80</a></li>
                        <li class="bt80-100"><a href="<?= $page; ?>?range=80-100">$80 - $100</a></li>
                        <li class="o100"><a href="<?= $page; ?>?range=100">OVER $100</a></li>
                        ...
                        <div class="sort_flower" style="margin:10px 14px 0 0;float: right;">
                            <div class="select-drop" style="width:250px;">
                                <select name="filter" onchange='this.form.submit()'>
                                    <option value="">--Sort Flowers By--</option>
                                    <option value="name_asc" <?PHP if ($filter == 'name_asc') { ?> selected="selected" <?php } ?>>Alphabetically (Ascending)</option>
                                    <option value="name_desc" <?PHP if ($filter == 'name_desc') { ?> selected="selected" <?php } ?>>Alphabetically (Descending)</option>
                                    <option value="price_asc" <?PHP if ($filter == 'price_asc') { ?> selected="selected" <?php } ?>>Price (Ascending)</option>
                                    <option value="price_desc" <?PHP if ($filter == 'price_desc') { ?> selected="selected" <?php } ?>>Price (Descending)</option>
                                </select>
                            </div>
                        </div>
                    </form>
