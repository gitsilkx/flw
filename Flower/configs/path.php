...
function getProductCodeById($product_id) {
    $result = mysql_query("SELECT `product_no` FROM products WHERE id=" . $product_id);
    $row = mysql_fetch_object($result);
...
function getProductNameById($product_id) {
    $result = mysql_query("SELECT `name` FROM products WHERE id=" . $product_id);
    $row = mysql_fetch_object($result);
    return $row->name;
    mysql_free_result($result);
}
...
function getProductImageById($product_id) {
    $result = mysql_query("SELECT `image` FROM products WHERE id=" . $product_id);
    $row = mysql_fetch_object($result);
    return $row->image;
    mysql_free_result($result);
}

function getProductTypeById($product_id) {
    $result = mysql_query("SELECT `type_id` FROM products WHERE id=" . $product_id);
    $row = mysql_fetch_object($result);
    return $row->type_id;
    mysql_free_result($result);
}
...
function getProductPriceById($product_id) {
    $result = mysql_query("SELECT `price` FROM products WHERE id=" . $product_id);
    $row = mysql_fetch_object($result);
    return $row->price;
    mysql_free_result($result);
}
...
function getProductDescriptionById($product_id) {
    $result = mysql_query("SELECT `description` FROM products WHERE id=" . $product_id);
    $row = mysql_fetch_object($result);
...
if ($current_page != "") {
    ...
    $r = mysql_query("select * from " . $prev . "contents where id='".$pg_id."'");
    //echo "select * from " . $prev . "contents where id='".$pg_id."'";
    ...
    if (@mysql_num_rows($r)) {
        ...
        $site_title = $ds['site_title'];
        $site_keys = $ds['meta_keys'];
        $site_desc = $ds['meta_desc'];
        $contents = $ds['contents'];
        $canonical = $ds['canonical'];
        $head_prm1 = $ds['header_prm1'];
        $head_prm2 = $ds['header_prm2'];
        $head_prm3 = $ds['header_prm3'];
        $head_prm4 = $ds['header_prm4'];
        $footer_content = $ds['footer_content'];
    } else {
        ...
        $canonical = '';
        $head_prm1 = $setting['header_msg'];
        $footer_content = $setting['footer_msg'];
        $head_prm2 = 'SEND';
        $head_prm3 = 'SAME DAY **';
